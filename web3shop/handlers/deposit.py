...
from data.config import locale, minimal_amount, gas_price, withdraw_address, currency_code, cryptobot_token
from keyboards.menu import crypto_check_keyboard, cryptobot_kb, cryptobot_form_kb, main_keyboard
from misc.db import get_price, lang_user, IsAdmin
...
async def cbot_deposit_amount(message: types.Message, state: FSMContext):
    ...
    if 1 > pay_amount < 10_000:
        minimal_formatted = f'10 {currency_code}'
        max_formatted = f'10 000 {currency_code}'
        error_text = locale[lang]['incorrect_amount'] + locale[lang]['amount_limit'].format(minimal_formatted,
                                                                                            max_formatted) + locale[lang]['top_up_enter']
